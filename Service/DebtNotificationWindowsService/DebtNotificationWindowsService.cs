...
namespace DebtNotificationWindowsService
{
    public partial class DebtNotificationWindowsService : ServiceBase
    {
        ...
        {
            Logger.WriteStart("DebtNotificationService stopped successfuly .");
        }
        ...
        private void PreparePaymentAcknowledgementData()
        {
            try
            {
                var garbagePayments = paymentAcknowledgementPhoneList.Where(p => p.Value.Date == DateTime.Now.Date.AddDays(-1)).ToList();
                int totalGarbage = 0;
                foreach (var item in garbagePayments)
                {
                    paymentAcknowledgementPhoneList.Remove(item.Key);
                    totalGarbage ++;
                }
                Logger.WriteInfo("PaymentAcknowledgement removed {0} records from garbage list .", totalGarbage );
                ...
                using (UmsDataContext context = new UmsDataContext())
                {
                    ...
                    Logger.WriteInfo("PaymentAcknowledgement fetch {0} records .", newPage.Count);
                        ...
                        foreach (var item in newPage)
                        {
                            ...
                            {
                                paymentAcknowledgementPhoneList.Add(item.PhoneNo, dt.Date);
                        ...
                        Logger.WriteInfo("PaymentAcknowledgement saved {0} records successfuly .", total);
